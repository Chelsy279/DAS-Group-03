...
$$
\begin{aligned}
\log(\text{Family\_Size}) &= \beta_0 + \beta_1 \log(\text{Income}) + \beta_2 \log(\text{Food\_Exp}) + \beta_3 \text{Head\_Sex} \\
&+ \beta_4 \text{Head\_Age} + \beta_5 \text{Household\_Type} + \beta_6 \log(\text{Floor\_Area}) \\
&+ \beta_7 \log(\text{House\_Age}) + \beta_8 \text{Bedrooms} + \beta_9 \text{Electricity}
\end{aligned}
$$


Family_Size: The expected value of family size (dependent variable).
log(Income): The logarithm of household income, measuring the economic level of the family.
log(Food_Exp): The logarithm of food expenditure, representing the household's spending on food.
Head_Sex: The gender of the household head (categorical variable, usually 0 for female and 1 for male).
Head_Age: The age of the household head (continuous variable), indicating the effect of the household head's age on family size.
Household_Type: The type of household (categorical variable), indicating different family structures such as single-parent or nuclear families.
log(House_Age): The logarithm of house age (with +1 to avoid taking the logarithm of 0), measuring the age of the dwelling.
Electricity: Whether the household has access to electricity (0 = no, 1 = yes), reflecting the household's infrastructure.
Bedrooms: The number of bedrooms in the household (continuous variable), reflecting the size of the dwelling.
log(Floor_Area): The logarithm of the floor area of the household (continuous variable), measuring the size of the living space.
...
Then Fit the second Poisson regression model

$$
\begin{aligned}
\log(\text{Family\_Size}) &= \beta_0 + \beta_1 \log(\text{Income}) + \beta_2 \log(\text{Food\_Exp}) \\
&+ \beta_3 \text{Head\_Sex} + \beta_4 \text{Head\_Age} + \beta_5 \text{Household\_Type} \\
&+ \beta_6 \log(\text{House\_Age}) + \beta_7 \text{Electricity}
\end{aligned}
$$
...
```
  ...
  Based on the model comparison and LRT test results, and considering the high correlation observed in the EDA heatmap, an interaction term between Income and Food_Exp was introduced. After comparing the original model with the model including the interaction term, it was found that the Deviance decreased by 47.118 after adding the interaction term, indicating that the inclusion of the interaction term significantly improved the model's fit. 
  Therefore, the conclusion can be made that introducing the interaction term effectively enhanced the model's explanatory power and fit:

$$
\begin{aligned}
\log(\text{Family\_Size}) &= \beta_0 + \beta_1 \log(\text{Income}) + \beta_2 \log(\text{Food\_Exp}) \\
&+ \beta_3 \log(\text{Income}) \times \log(\text{Food\_Exp}) + \beta_4 \text{Head\_Sex} \\
&+ \beta_5 \text{Head\_Age} + \beta_6 \text{Household\_Type} + \beta_7 \log(\text{House\_Age}) \\
&+ \beta_8 \text{Electricity}
\end{aligned}
$$


The explanation for each variable shows below:
Family_size: The expected value of family size (dependent variable).
log(Income): The logarithm of household income, measuring the economic level of the family.
log(Food_Exp): The logarithm of food expenditure, representing the household's spending on food.
log(Income) * log(Food_Exp): The interaction term between income and food expenditure, measuring their joint effect on family size.
Head_Sex: The gender of the household head (categorical variable, usually 0 for female and 1 for male).
Head_Age: The age of the household head (continuous variable), indicating the effect of the household head's age on family size.
Household_Type: The type of household (categorical variable), indicating different family structures such as single-parent or nuclear families.
log(House_Age: The logarithm of house age (with +1 to avoid taking the logarithm of 0), measuring the age of the dwelling.
...
X2 <- sum(resid(poisson_model4, type = "pearson")^2)
dp <- X2 / poisson_model4$df.res
summary(poisson_model4, dispersion = dp)
pred <- predict(poisson_model4, type = "response")
stand.resid <- rstandard(model = poisson_model4, type = "pearson") # Standardised Pearson residuals
par(mfrow=c(1,2))
plot(x = pred, y = stand.resid, xlab = "Predicted count", ylab = "Standardised Pearson residuals",
     main = "Regular likelihood", ylim = c(-5,5))
abline(h = c(-3,-2, 0, 2, 3), lty = "dotted", col = "red")
...
# Fit the third Poisson regression model
poisson_model5 <- glm(Family_Size ~ 
                      log(Income) * log(Food_Exp) +
                      Head_Sex + 
                      Head_Age + 
                      Household_Type +
                      ...
                      family = quasipoisson(link = "log"),
                      data = FIES)  # Quasi-Poisson model
pred <- predict(poisson_model5, type = "response")
stand.resid <- rstandard(model = poisson_model5, type = "pearson") # Standardised Pearson residuals
plot(x = pred, y = stand.resid, xlab = "Predicted count", ylab = "Standardised Pearson residuals",
     main = "Quasi-likelihood", ylim = c(-5,5))
abline(h = c(-3,-2, 0, 2, 3), lty = "dotted", col = "red")
```
...
# Conclusion
  Based on the results of Relative Risk from the model, the following key conclusions can be summarized:
```{r}
#| echo: false
