...
From @tbl-y-household, it summarized family size statistics across three household types. Single-family households have the smallest average family size  (mean = 4.14, median = 4.00) with a large sample size (1311). Extended families show significantly larger family sizes (mean=5.88, median = 5.00) and a broader spread (max = 16). Households with two or more nonrelated members report the highest average (mean = 6.89, median - 6.00), but with a extremely small sample size (9) weakens reliability.
...
@fig-y-electricity shows that both groups (with and without electricity) share the same median family size of 4, indicating a common central tendency. However, households with electricity display a much wider range, extending to a maximum of 16 members which far beyond the upper limit of households without electricity which have maximum 6. The IQR for electricity-access families is slightly broader, reflecting higher variability in family size compared to non-electricity households.
