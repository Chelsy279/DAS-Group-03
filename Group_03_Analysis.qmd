...
```
var/mean=5.28/4.68=1.13¡Ö1 indicates that there is no overdispersion problem, so Poisson regression model may be appropriate.
...
From @fig-his-num, we can see the distributions of household income and household expenditure exhibit a noticeable right skew, suggesting that applying a log transformation may be beneficial when fitting a Poisson model. This also indicates that most households have relatively low income, while a few have extremely high income, reflecting a significant income disparity within the population.  
